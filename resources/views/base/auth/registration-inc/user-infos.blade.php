...
@section('user-content')
    <form method="POST" action="{{ route('to-company-infos') }}" style="width: 100%">
        @csrf
        ...
        <div class="input_container">
            ...
            <input placeholder="name@mail.com" value="{{ isset($data['email']) ? $data['email'] : '' }}" required
                title="Inpit title" name="email" type="email" class="input_field" id="email_field">
        </div>
        <br>
        <div class="input_container">
            <svg fill="none" viewBox="0 0 24 24" height="24" width="24" xmlns="http://www.w3.org/2000/svg"
            ...
            <input placeholder="MOT DE PASSE" value="{{ isset($data['password']) ? $data['password'] : '' }}"
                name="password"    type="password" class="input_field" id="password">
        </div>
        <div hidden id="password-error" class="alert alert-danger error-message" role="alert">
        </div>
        <br>
        <div class="input_container">
            <svg fill="none" viewBox="0 0 24 24" height="24" width="24" xmlns="http://www.w3.org/2000/svg"
            ...
            <input placeholder="CONFIRMER MOT DE PASSE"
               ...
               required title="Input title" name="confirm-password"
                type="password" class="input_field" id="confirm_password">
        </div>
        <div hidden id="password-mismatch-error" class="alert alert-danger error-message" role="alert">
        </div>
        <br>
        ...
        <div class="actions">
            ...
            <button title="Sign In" type="submit"  id="next-btn" class="sign-in_btn next-btn">
                <span>SUIVANT</span>
            </button>
        </div>
    </form>
    ...
    addEventListener("DOMContentLoaded", (event) => {
        ...
        confirmPasswordInput.addEventListener('input', () => {
            if (passwordInput.value !== confirmPasswordInput.value) {
                passwordMismatchError.textContent = 'Passwords not match';
                passwordMismatchError.removeAttribute('hidden');
            } else {
                passwordMismatchError.textContent = '';
                passwordMismatchError.setAttribute('hidden', true);
            }
        });
